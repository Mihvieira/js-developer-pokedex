...
function transformData(pokeDetail:any):Pokemon {
  ...
  let types = pokeDetail.types.map((typeSlot: { type: { name: any; }; }) => typeSlot.type.name);
  let [type] = types;
  ...
  const stats = pokeDetail.stats.map((statsSlot: { stat: { name: any; }; }) => statsSlot.stat.name);
  let [stat] = stats;
  const statsNumber = pokeDetail.stats.map((baseSlot: { base_stat: any; }) => baseSlot.base_stat);
  ...
  pokemon.types = types;
  pokemon.type = type;
  const moves = pokeDetail.moves.map((moveSlot: { move: { name: any; }; }) => moveSlot.move.name);
  const [move] = moves;
